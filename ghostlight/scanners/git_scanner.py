    ...
    def scan(self, target: str, config: ScanConfig) -> Iterable[Finding]:
        ...
        for blob in repo.head.commit.tree.traverse():
            ...
            if ai_mode != "off" and detailed:
                try:
                    logger.info(
                        f"AI filter enabled (mode={ai_mode}) for blob {blob.path} with {len(filtered)} detections pre-AI"
                    )
                except Exception:
                    logger.debug("AI filter start log failed (git)")
                ai_verified = []
